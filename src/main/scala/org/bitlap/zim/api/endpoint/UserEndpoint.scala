...
import org.bitlap.zim.domain.{ AddInfo, ChatHistory, FriendAndGroupInfo, FriendList, UploadResult, UserVo, ZimError }
...
trait UserEndpoint extends ApiErrorMapping {
  ...
  private[api] lazy val chatLogEndpoint
    ...
    endpoint.post
      ...
      .name("获取聊天记录")
      .description(userDescriptionGetResource)
      .out(streamBody(AkkaStreams)(Schema(Schema.derived[ChatHistory].schemaType), CodecFormat.Json()))
      .errorOut(oneOf(statusInternalServerError, statusDefault))
  ...
  private[api] lazy val createUserGroupEndpoint
    : Endpoint[FriendGroup, ZimError, Source[ByteString, Any], Any with AkkaStreams] =
    endpoint.post
      .in(userResource / "createUserGroup")
      .in(jsonBody[FriendGroup])
      .name("用户创建好友分组")
      .description(userDescriptionGetResource)
      .out(streamBody(AkkaStreams)(Schema(SchemaType.SInteger), CodecFormat.Json()))
      .errorOut(oneOf(statusInternalServerError, statusDefault))
