...
import org.bitlap.zim.domain.model.{ AddMessage, GroupList, Receive, User }
 ...
 * 注意：sim项目controller层的逻辑都需要下沉到这里，而不只是实现原UserService的逻辑
 * @author 梦境迷离
 * @since 2021/12/25
 * @version 1.0
 */
trait UserApplication extends BaseApplication[User] {
   ...
   * @return
   */
  def countUnHandMessage(uid: Int, agree: Integer): stream.Stream[Throwable, Int]
   ...
   * @return
   */
  def countGroup(groupName: String): stream.Stream[Throwable, Int]
   ...
   * @return
   */
  def findGroup(groupName: String): stream.Stream[Throwable, GroupList]
   ...
   * @return
   */
  def countUsers(username: String, sex: Integer): stream.Stream[Throwable, Int]
   ...
   * @return
   */
  def findUsers(username: String, sex: Integer): stream.Stream[Throwable, User]
   ...
   * @return
   */
  def findHistoryMessage(user: User, mid: Int, `type`: String): stream.Stream[Throwable, ChatHistory]
