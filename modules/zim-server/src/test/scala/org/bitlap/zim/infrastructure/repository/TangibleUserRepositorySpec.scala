...
final class TangibleUserRepositorySpec extends TangibleUserRepositoryConfigurationSpec {
  ...
  it should "findUserByGroupId by gid" in {
    import org.bitlap.zim.domain.model.{ GroupMember, User }
  ...
  it should "findUsersByFriendGroupIds by fgid" in {
    import org.bitlap.zim.domain.model.{ AddFriend, User }
    val actual: Chunk[User] = unsafeRun(
      (for {
  ...
  it should "findUserGroup by uid and mid" in {
    ...
    val actual: Option[Int] = unsafeRun(
      (for {
        id1 <- TangibleUserRepository.saveUser(mockUser)
        id2 <- TangibleUserRepository.saveUser(mockUser.copy(username = "myname"))
        _ <- TangibleFriendGroupRepository.createFriendGroup(model.FriendGroup(1, id1.toInt, "groupname"))
        _ <- TangibleFriendGroupRepository.createFriendGroup(model.FriendGroup(2, id2.toInt, "groupname"))
  ...
  it should "changeGroup by groupId and table id" in {
    ...
    val actual: Option[Int] = unsafeRun(
      (for {
        id1 <- TangibleUserRepository.saveUser(mockUser)
        id2 <- TangibleUserRepository.saveUser(mockUser.copy(username = "myname"))
        _ <- TangibleFriendGroupRepository.createFriendGroup(model.FriendGroup(0, id1.toInt, "1-groupname"))
        _ <- TangibleFriendGroupRepository.createFriendGroup(model.FriendGroup(0, id1.toInt, "1-groupname"))
        _ <- TangibleFriendGroupRepository.createFriendGroup(model.FriendGroup(0, id2.toInt, "2-groupname"))
  ...
  it should "find FriendGroup by id" in {
    import org.bitlap.zim.domain.model
    val actual: Option[model.FriendGroup] = unsafeRun(
      (for {
        _ <- TangibleFriendGroupRepository.createFriendGroup(model.FriendGroup(0, 1, "1-groupname"))
  ...
  it should "findFriendGroupsById by uid" in {
    import org.bitlap.zim.domain.model
    val actual: Chunk[model.FriendGroup] = unsafeRun(
      (for {
        _ <- TangibleFriendGroupRepository.createFriendGroup(model.FriendGroup(0, 1, "1-groupname"))
        _ <- TangibleFriendGroupRepository.createFriendGroup(model.FriendGroup(0, 1, "2-groupname"))
